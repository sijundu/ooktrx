...
class FrameStackRxTester(val c: FrameStackRx[UInt]) extends DspTester(c) {
  ...
  while(numberOfSteps < 2000) {
    if(numberOfSteps % 10 == 3) {
      poke(c.io.in.bits, Random.nextInt(math.pow(2, c.frameWidth).toInt).asUInt)
      frameValid = true.B
    } else {
      frameValid = false.B
    }
...
class FrameStackRxSpec extends FreeSpec with Matchers {
  val frameWidthNb = 20
  val StackSizeNb = 16
  val frameIn = FixedPoint(frameWidthNb.W)
  "Frame Stack for Rx first test" in {
    val gen = () => new FrameStackRx(DspReal(frameIn), frameWidth = frameWidthNb, StackSize = StackSizeNb)
