...
#endif
#include <iostream>
class FrameSync_api_t: public sim_api_t<VerilatorDataWrapper*> {
public:
    FrameSync_api_t(VFrameSync* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_syncOk)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["FrameSync.reset"] = 0;
    }
#if VM_TRACE
...
static FrameSync_api_t * _Top_api;
double sc_time_stamp () { return _Top_api->get_time_stamp(); }
int main(int argc, char **argv, char **env) {
    Verilated::commandArgs(argc, argv);
    VFrameSync* top = new VFrameSync;
    std::string vcdfile = "test_run_dir/frame_sync_first_test/ooktrx.FrameSyncSpec945290574/FrameSync.vcd";
